...
<div class="container text-center">
	...
	<h1 class="text-uppercase"><b>inmuebles</b></h1>
	<hr>
	<div class="space-20"></div>
</div>
               ...
               <!-- Brand and toggle get grouped for better mobile display -->
                ...
                <div class="collapse navbar-collapse " ID="bs-example-navbar-collapse-2">
                    <ul class="nav navbar-nav">
                        <li><a href="<?php echo $url->getVars($_SERVER['REQUEST_URI'], '&select=venta&pagination=0'); ?>"> Inmuebles en venta</a></li>
                        <li><a href="<?php echo $url->getVars($_SERVER['REQUEST_URI'], '&select=alquilerl&pagination=0'); ?>"> Inmuebles en alquiler</a></li> 
...
if (!empty($_GET['select']) or isset($_GET['select'])){
    ...
    if ($select == 'venta'){
        $where = "WHERE finality = 'venta' AND visibility = 'Y' AND status IS NULL";
    }
    else if ($select == 'alquiler'){
        $where = "WHERE finality = 'alquiler' AND visibility = 'Y' AND status IS NULL";
    }
    else{
        $where = "WHERE visibility = 'Y' AND status IS NULL";
    }
...
if( $properties and $rows > 0){
    ...
    while( $reg = $properties->fetch_assoc() ){
        ...
        if($reg['finality'] == 'venta-alquiler'){
            $finality = 'venta o alquiler';

            if($reg['hidden_value_total'] == 'Y' and !empty( $reg['value_total'])){
                $venta = '<p>venta:<strong class="text-iguacu"> R$ '.decimalMoeda( $reg['value_total']).' </strong> - <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_total'] == 'Y' and empty( $reg['value_total']))
            {
                $venta = '<p>Venta:<strong class="text-iguacu"> No informado </strong> - <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_total'] == 'N' and !empty( $reg['value_total']))
            {
                $venta = '<p>Venta:<strong class="text-iguacu"> R$ '.decimalMoeda( $reg['value_total']).' </strong></p>';
            }
            else if($reg['hidden_value_total'] == 'N' and empty( $reg['value_total']))
            {
                $venta = '<p>Venta:<strong class="text-iguacu"> No informado </strong></p>';
            }
            else
            {
                $venta = '<p>Venta:<strong class="text-iguacu"> Indefinido </strong></p>';
            }

            if($reg['hidden_value_monthly'] == 'Y' and !empty($reg['value_monthly']))
            {
                $alquiler = '<p>Alquiler:<strong class="text-iguacu"> R$ '.decimalMoeda($reg['value_monthly']).' </strong> - <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_monthly'] == 'Y' and empty($reg['value_monthly']))
            {
                $alquiler = '<p>Alquiler:<strong class="text-iguacu"> Não informado </strong> - <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_monthly'] == 'N' and !empty($reg['value_monthly']))
            {
                $alquiler = '<p>Alquiler:<strong class="text-iguacu"> R$ '.decimalMoeda($reg['value_monthly']).' </strong></p>';
            }
            else if($reg['hidden_value_monthly'] == 'N' and empty($reg['value_monthly']))
            {
                $alquiler = '<p>Alquiler:<strong class="text-iguacu"> Não informado </strong></p>';
            }
            else
            {
                $alquiler = '<p>Alquiler:<strong class="text-iguacu"> Indefinido </strong></p>';
            }

        }
        else if( $reg['finality'] == 'venta' )
        {
            $finality = 'venta';
            if($reg['hidden_value_total'] == 'Y' and !empty( $reg['value_total']))
            {
            ...
            {
                $venta = '<p>venta: <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_total'] == 'N' and !empty( $reg['value_total']))
            {
                $venta = '<p>venta:<strong class="text-iguacu"> R$ '.decimalMoeda( $reg['value_total']).' </strong></p>';
            ...
            {
                $venta = '<p>venta:<strong class="text-iguacu"> Não informado </strong></p>';
            }
            else
            {
                $venta = '<p>venta:<strong class="text-iguacu"> Indefinido </strong></p>';
        ...
        else if( $reg['finality'] == 'alquiler' or $reg['finality'] == 'temporada' or $reg['finality'] == 'arrendamento')
        {
            $finality = 'alquiler';
            if($reg['hidden_value_monthly'] == 'Y' and !empty($reg['value_monthly']))
            {
                $alquiler = '<p>alquiler:<strong class="text-iguacu"> R$ '.decimalMoeda($reg['value_monthly']).' </strong> - Sob consulta</p>';
            }
            else if($reg['hidden_value_monthly'] == 'Y' and empty($reg['value_monthly']))
            {
                $alquiler = '<p>alquiler:  <small>Sob consulta</small></p>';
            }
            else if($reg['hidden_value_monthly'] == 'N' and !empty($reg['value_monthly']))
            {
                $alquiler = '<p>alquiler:<strong class="text-iguacu"> R$ '.decimalMoeda($reg['value_monthly']).' </strong></p>';
        ...
        else
        {
            ...
            $venta = '<p>Indefinido</p>';
            $alquiler = '<p>Indefinido</p>';
        }
